...
function App() {
  ...
  const sauvegarderTarifs = async (e) => {
    ...
    const { error } = await supabaseClient
      .from('configuration')
      .upsert({ id: 'tarifs', donnees: tarifs });

    if (error) {
      alert(`Erreur : ${error.message}`);
    } else {
      alert('Tarifs sauvegardés dans Supabase !');
    }
  };
  ...
  const sauvegarderClubInfo = async (e) => {
    ...
    const { error } = await supabaseClient
      .from('configuration')
      .upsert({ id: 'clubInfo', donnees: clubInfo });

    if (error) {
      alert(`Erreur : ${error.message}`);
    } else {
      alert('Informations du club sauvegardées dans Supabase !');
    }
  };
